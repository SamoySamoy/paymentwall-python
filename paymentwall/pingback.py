...
class Pingback(Paymentwall):
    ...
    def is_ip_address_valid(self) -> bool:
        ...
        """
        ips_whitelist = [
            '174.36.92.186', '174.36.96.66', '174.36.92.187', '174.36.92.192', '174.37.14.28',
    ...
    def is_parameters_valid(self) -> bool:
        ...
        """
        required_params = ['uid', 'currency', 'type', 'ref', 'sig'] if self.get_api_type() == self.API_VC else ['uid', 'goodsid', 'type', 'ref', 'sig']
        errors_number = sum(1 for field in required_params if field not in self.parameters)
    ...
    def get_product(self) -> Product:
        ...
        return Product(
            ...
            currency_code=None,
            name=None,
            product_type=Product.TYPE_SUBSCRIPTION if self.get_product_period_length() > 0 else Product.TYPE_FIXED,
    ...
    def calculate_signature(self, params: Dict[str, Any], secret: Optional[str], version: int) -> str:
        ...
        base_string = ''
        keys = sorted(params.keys()) if version in (self.SIGNATURE_VERSION_2, self.SIGNATURE_VERSION_3) else params.keys()
        for key in keys:
            value = params[key]
        ...
        base_string += secret
        return self.hash(base_string, 'sha256' if version == self.SIGNATURE_VERSION_3 else 'md5')
